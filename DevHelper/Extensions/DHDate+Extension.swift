    ...
    func toStringWithRelativeTime(strings: [RelativeTimeStringType: String]? = nil) -> String {
        ...
        if sec < 60 {
            if sec < 10 {
                ...
                let string: String
                if isPast {
                    string = strings?[.secondsPast] ?? NSLocalizedString("%.f секунд назад", comment: "Date format")
                } else {
                    string = strings?[.secondsFuture] ?? NSLocalizedString("в течении %.f секунд", comment: "Date format")
                }
        ...
        if min < 60 {
            if min == 1 {
                if isPast {
                    ...
                    return strings?[.oneMinuteFuture] ?? NSLocalizedString("в течении %.f минуты", comment: "Date format")
                }
            } else {
                let string: String
                if isPast {
                    string = strings?[.minutesPast] ?? NSLocalizedString("%.f минут назад", comment: "Date format")
                } else {
                    string = strings?[.minutesFuture] ?? NSLocalizedString("в течении %.f минут", comment: "Date format")
                }
        ...
        if hr < 24 {
            if hr == 1 {
                ...
                let string: String
                if isPast {
                    string = strings?[.hoursPast] ?? NSLocalizedString("%.f часов назад", comment: "Date format")
                } else {
                    string = strings?[.hoursFuture] ?? NSLocalizedString("в течении %.f часов", comment: "Date format")
                }
        ...
        if d < 7 {
            if d == 1 {
                ...
                let string: String
                if isPast {
                    string = strings?[.daysPast] ?? NSLocalizedString("%.f дней назад", comment: "Date format")
                } else {
                    string = strings?[.daysFuture] ?? NSLocalizedString("в течении %.f дней", comment: "Date format")
                }
